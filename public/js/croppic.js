...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		bindImgUploadControl: function(){
            ...
            var formHtml = '<form class="' + that.id + '_imgUploadForm" style="visibility: hidden;position:absolute;">  <input type="file" name="filename" id="' + that.id + '_imgUploadField">  </form>';
			...
			that.form.find('input[type="file"]').change(function(){
				...
				if(that.options.processInline){			
					...
					if ($("#upd_title").val() != '') { // передаем описание файла
						that.options.uploadData['upd_title'] = $("#upd_title").val();
						$("#upd_title").prop('disabled', true);
						//$('#upd_title').val('');
					}else{

						that.options.uploadData['upd_title'] = 'title';
						$('#upd_title').val('title');
						$("#upd_title").prop('disabled', true);
		...
		afterUpload: function(data){
            ...
            if (response.status == 'success') {
                ...
                that.file_hash = response.file_hash;
		...
		createModal: function(){
			...
			var marginTop = that.windowH/2-that.obj.height()/2;
			var modalHTML =  '<div id="croppicModal">'+'<div id="croppicModalObj" style="width:'+ that.obj.width() +'px; height:'+ that.obj.height() +'px; margin:0 auto; margin-top:'+ marginTop +'px; position: relative;"> </div>'+'</div>';
		...
		initCropper: function(){
			...
			that.img.wrap('<div class="cropImgWrapper" style="overflow:hidden; z-index:1; position:absolute; width:'+that.obj.width()+'px; height:'+that.obj.height()+'px;"></div>');
		...
		initialScaleImg:function(){
			...
			that.img.css({'left': -(that.imgW -that.obj.width())/2, 'top': -(that.imgH -that.obj.height())/2, 'position':'relative'});
			if(that.options.imgEyecandy){ that.imgEyecandy.css({'left': -(that.imgW -that.obj.width())/2, 'top': -(that.imgH -that.obj.height())/2, 'position':'relative'}); }
		...
		initDrag:function(){
			...
			that.img.on("mousedown touchstart", function(e) {
				...
				that.img.css('z-index', 1000).on("mousemove touchmove", function(e) {
					...
					if (that.obj.height() < that.imgH) {
						if (parseInt(that.img.css('top')) > 0) { that.img.css('top', 0); if (that.options.imgEyecandy) { that.imgEyecandy.css('top', 0);}}
						var maxTop = -( that.imgH - that.obj.height()); if (parseInt(that.img.css('top')) < maxTop) { that.img.css('top', maxTop); if (that.options.imgEyecandy) { that.imgEyecandy.css('top', maxTop); }}
					}else{
						if (parseInt(that.img.css('top')) < 0) { that.img.css('top', 0); if (that.options.imgEyecandy) { that.imgEyecandy.css('top', 0); }}
						var maxTop =  that.obj.height() - that.imgH; if (parseInt(that.img.css('top')) > maxTop) { that.img.css('top', maxTop);if (that.options.imgEyecandy) {that.imgEyecandy.css('top', maxTop); }}
					}

					if (that.obj.width() < that.imgW) {
						if( parseInt( that.img.css('left')) > 0 ){ that.img.css('left',0); if(that.options.imgEyecandy){ that.imgEyecandy.css('left', 0); }}
						var maxLeft = -( that.imgW-that.obj.width()); if( parseInt( that.img.css('left')) < maxLeft){ that.img.css('left', maxLeft); if(that.options.imgEyecandy){ that.imgEyecandy.css('left', maxLeft); } }
					}else{
						if( parseInt( that.img.css('left')) < 0 ){ that.img.css('left',0); if(that.options.imgEyecandy){ that.imgEyecandy.css('left', 0); }}
						var maxLeft = ( that.obj.width() - that.imgW); if( parseInt( that.img.css('left')) > maxLeft){ that.img.css('left', maxLeft); if(that.options.imgEyecandy){ that.imgEyecandy.css('left', maxLeft); } }
					}
		...
		zoom :function(x){
			...
			if( newWidth < that.obj.width() || newHeight < that.obj.height()){
				
				if( newWidth - that.obj.width() < newHeight - that.obj.height() ){ 
					newWidth = that.obj.width();
					newHeight = newWidth/ratio;
				}else{
					newHeight = that.obj.height();
			...
			var maxTop = -( newHeight-that.obj.height()); if( newTop < maxTop){	newTop = maxTop;	}
			var maxLeft = -( newWidth-that.obj.width()); if( newLeft < maxLeft){	newLeft = maxLeft;	}
		...
		crop:function(){
			...
			var cropData = {
					...
					cropW: that.obj.width(),
					cropH: that.obj.height(),				
